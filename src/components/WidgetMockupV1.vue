<template>
  <div class="mockup-page">
    <div class="mockup-label">Variation 1 — Gear Icon + Tabs</div>
    <div class="mockup-description">Gear icon in the header opens a tabbed config view. Back button returns to signals.</div>
        ...
        <div class="ai-widget-inner">
          ...
          <div class="ai-widget-header">
            <svg width="0" height="0" style="position: absolute;">
              <defs>
                <linearGradient id="ai-sparkle-gradient-v1" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <div class="ai-widget-icon-border">
              <div class="ai-widget-icon-inner">
                <PendoIcon type="sparkle" :size="18" stroke="url(#ai-sparkle-gradient-v1)" :stroke-width="1.5" />
              </div>
            </div>
            <span class="ai-widget-title">Dashboard signals</span>

            <div class="header-actions">
              <button v-if="configMode" class="icon-btn" @click="configMode = false" title="Back to signals">
                <PendoIcon type="arrow-left" :size="16" :stroke-width="2" />
              </button>
              <button v-else class="icon-btn" @click="configMode = true" title="Configure signals">
                <PendoIcon type="settings" :size="16" :stroke-width="1.5" />
              </button>
          ...
          <div v-if="!configMode" class="signals">
            <div class="signal">
          ...
          <template v-else>
            <div class="tabs">
              ...
              >
                <PendoIcon :type="tab.icon" :size="14" :stroke-width="1.5" />
                {{ tab.label }}
              </button>
            </div>

            <!-- Tab: Customize -->
            <div v-if="activeTab === 'customize'" class="tab-content">
              <p class="section-label">Select widgets to use as context for signals:</p>
              ...
              <div class="tab-actions">
                <PendoButton type="secondary" label="Cancel" @click="configMode = false" />
                <PendoButton type="primary" label="Save" />
              </div>
            </div>

            <!-- Tab: Auto Slack -->
            <div v-if="activeTab === 'auto-slack'" class="tab-content">
            ...
            <!-- Tab: Manual Slack -->
            <div v-if="activeTab === 'manual-slack'" class="tab-content">
              <div class="form-fields">
                <div class="form-field">
                  <label class="form-label">Channel</label>
                  <PendoDropdownTrigger variant="standard" model-value="#product-insights" placeholder="Select channel" />
...
const tabs = [
  ...
  { id: 'auto-slack', label: 'Auto Slack', icon: 'slack' },
  { id: 'manual-slack', label: 'Share to Slack', icon: 'send' }
]
...
.tab--active {
  ...
  border-bottom-color: var(--teal-70, #128297);
}
